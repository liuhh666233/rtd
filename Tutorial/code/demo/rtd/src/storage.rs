...
impl Display for StorageError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match &self {
            ...
            ParseItem(source) => write!(f, "Rtd storage parse item error: {}", source),
            ItemNoExist(id) => write!(f, "Rtd storage item not exist: {}", id),
        }
    }
}
...
impl From<ParseItemError> for StorageError {
    fn from(value: ParseItemError) -> Self {
...
/// Calculate the offset of the line by id
...
pub fn get_item_by_id(id: u32) -> Result<Item> {
    ...
    item_str.map_or(Err(StorageError::ItemNoExist(id)), |item_str| {
